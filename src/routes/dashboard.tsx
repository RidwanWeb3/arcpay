  ...
  head: () => ({
    meta: [
      { title: "System Dashboard — ArcPay Agent" },
      {
        name: "description",
        content:
          "Aggregated telemetry for the ArcPay Agent network: agent activity, payment volume, service usage and transaction frequency.",
      },
      { property: "og:title", content: "System Dashboard — ArcPay Agent" },
      {
        property: "og:description",
        content: "Command-center view of agent-driven payment flow on ARC.",
      },
...
function seriesToLabeled(input: number[] | LabeledSeries[]): LabeledSeries[] {
  if (input.length === 0) return [];
  if (typeof input[0] === "number") {
    return (input as number[]).map((v, i) => ({ label: `H${i}`, value: v }));
  }
  return input as LabeledSeries[];
}
...
function DashboardPage() {
  ...
  return (
    <PageShell
      ...
      actions={
        <>
          <LinkButton to="/activity" size="sm" variant="primary">
            LIVE ACTIVITY
          </LinkButton>
          <LinkButton to="/agents" size="sm">
            AGENT REGISTRY
          </LinkButton>
        </>
      }
    >
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          value={`${volume.toFixed(4)} USDC`}
          sub="SIMULATED"
          tone="cyan"
        />
        <MetricCard
          label="SERVICES ONLINE"
          value={String(SERVICES.filter((s) => s.status === "AVAILABLE").length)}
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <LineChartPanel title="PAYMENT VOLUME" data={seriesToNumbers(DASHBOARD_SERIES.paymentVolume)} unit=" USDC" />
        <BarChartPanel title="SERVICE USAGE" data={seriesToLabeled(DASHBOARD_SERIES.serviceUsage)} />
        <HistogramPanel title="TRANSACTION FREQUENCY" data={seriesToNumbers(DASHBOARD_SERIES.txFrequency)} />
      </div>
